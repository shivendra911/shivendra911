<!-- ===================== HEADER ===================== -->
<h1 align="center">Hi 👋, I'm Shivendra Pratap</h1>
<h3 align="center">
BTech CSE Student | Machine Learning Minor | Aspiring Software Engineer
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=00F7FF&center=true&vCenter=true&lines=Actively+Seeking+Internships+%26+Placements;Machine+Learning+%7C+DSA+%7C+Linux;AI+%26+Automation+Project+Builder;Consistent+Learner+%26+Problem+Solver" />
</p>

---

## 🙋‍♂️ About Me (Recruiter View)

- 🎓 BTech **Computer Science Engineering** student  
- 🤖 **Machine Learning Minor**
- 🧠 Strong in **DSA & problem solving**
- 🛰️ Working on **Satellite Image Segmentation (Computer Vision)**
- 🧪 Building **AI-driven & automation systems**
- 🐧 Hands-on with **Linux & Shell scripting**
- 🎯 **Actively seeking internships & full-time roles**

---

## 🛠️ Technical Skills

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,cpp,java,linux,bash,git,github,mysql,tensorflow,pytorch,flask,vscode&theme=dark" />
</p>

---

## 🚀 Featured Projects

### 🛰️ Automated Road Extraction Using Satellite Images
- Deep learning–based **image segmentation**
- Migrated model from **TensorFlow → PyTorch**
- Enabled **GPU acceleration**
- Real-world application in **GIS & smart cities**

**Tech:** Python, PyTorch, CNNs, Computer Vision  

---

### 🤖 AI-Enabled Guest List & Event Management System
- AI chatbot for **guest handling & verification**
- Automated event workflows
- Backend using **Flask**
- Real-world AI automation focus

**Tech:** Python, Flask, ML, APIs  

---

### 🐧 Linux System Administration & Shell Automation
- Developed **multiple shell scripts**
- User management, backups & automation
- RHCSA-level practical exposure

**Tech:** Linux, Bash, Git  

---

## 🎯 Current Focus (Placement Aligned)

- 🔹 Data Structures & Algorithms (Interview prep)
- 🔹 Machine Learning (Classification, Regression, CNNs)
- 🔹 Linux System Administration
- 🔹 System Design fundamentals
- 🔹 Building production-style projects

---


## 🧠 System Design (Currently Learning)

I have **recently started learning System Design** and am actively building a strong foundation to design **scalable and reliable systems**.

### 🔹 What I’m Learning
- Basics of scalability and performance
- Latency vs throughput
- Introduction to CAP theorem
- Fundamentals of caching
- Basics of load balancing
- SQL vs NoSQL (when to use what)
- Introduction to microservices architecture

### 🔹 Learning Approach
- Studying real-world system design problems
- Understanding design trade-offs step by step
- Practicing simple design problems before scaling
- Using Linux to understand system-level behavior

### 🔹 Beginner Design Problems (In Progress)
- Designing a URL Shortener (basic)
- Designing a simple Chat Application
- Designing a Rate-Limited API (conceptual)

🎯 **Goal:** Progress from fundamentals to **interview-ready System Design** through consistent practice and real implementations.

---


## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=shivenda911&show_icons=true&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=shivenda911&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=shivenda911&layout=compact&theme=tokyonight&hide_border=true" />
</p>

---

## 💻 LeetCode Stats

<p align="center">
  <img src="https://leetcard.jacoblin.cool/shivenda911?theme=dark&font=Roboto&ext=contest" />
</p>

---

## 🏆 GitHub Achievements

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=shivenda911&theme=tokyonight&no-frame=true&row=1" />
</p>

---

## 📜 Certifications

- ✅ Machine Learning (Coursera)
- ✅ Core Java Programming
- ✅ Linux System Administration
- ✅ Data Structures & Algorithms (In Progress)

---

## 📄 Resume

<p align="center">
  <a href="[https://drive.google.com/your-resume-link](https://drive.google.com/file/d/1ZFAPv5P_IOnago_rmhpa1WZ5BjZ7EMBU/view?usp=sharing)" target="_blank">
    <img src="https://img.shields.io/badge/Resume-View-blue?style=for-the-badge" />
  </a>
</p>

---

## 🔗 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/shivendra--pratap" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&style=for-the-badge" />
  </a>
  <a href="mailto:shivendrapratapsp3@gmail.com">
    <img src="https://img.shields.io/badge/Email-red?logo=gmail&style=for-the-badge" />
  </a>
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=shivenda911&label=Profile%20Views&color=blue&style=flat" />
</p>

---

<p align="center">
⭐ <i>"Focused on learning, building, and delivering real-world impact."</i>
</p>
